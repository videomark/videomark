...
export default class VideoHandler {
  ...
  get_receive_buffer() {
    ...
    if (this.handler instanceof IIJTypeHandler) receive = IIJTypeHandler.get_receive_buffer();
  ...
  get_video_title() {
    ...
    if (this.handler.get_video_title instanceof Function) title = this.handler.get_video_title();
    ...
    if (!title) {
      ...
      if (separator !== -1) title = title.substr(0, separator).trim();
  ...
  get_video_thumbnail() {
    ...
    if (!thumbnail && og_image) thumbnail = og_image.content;
  ...
  get_view_count() {
    ...
    if (this.handler.get_view_count instanceof Function) view_count = this.handler.get_view_count();
  ...
  get_play_list_info() {
    ...
    if (this.handler instanceof YouTubeTypeHandler) list = YouTubeTypeHandler.get_play_list_info();
  ...
  get_throughput_info() {
    ...
    if (this.handler instanceof YouTubeTypeHandler) list = YouTubeTypeHandler.get_throughput_info();
  ...
  get_codec_info() {
    ...
    if (this.handler instanceof YouTubeTypeHandler) info = YouTubeTypeHandler.get_codec_info();
    else if (this.handler.get_codec_info instanceof Function) info = this.handler.get_codec_info();
  ...
  get_total_frames(video) {
    if (this.handler instanceof AbemaTVLiveTypeHandler) return this.handler.get_total_frames();
    else return video.getVideoPlaybackQuality().totalVideoFrames;
  }

  get_dropped_frames(video) {
    if (this.handler instanceof AbemaTVLiveTypeHandler) return this.handler.get_dropped_frames();
    else return video.getVideoPlaybackQuality().droppedVideoFrames;
  }

  is_main_video(video) {
    if (this.handler.is_main_video instanceof Function) return this.handler.is_main_video(video);
    return true;
  }

  is_cm(video) {
    if (this.handler.is_cm instanceof Function) return this.handler.is_cm(video);
    return false;
  }

  is_limited() {
    if (this.handler.is_limited instanceof Function) return this.handler.is_limited();
    return false;
  }
  ...
  set_quality(bitrate) {
    if (this.handler.set_quality instanceof Function) this.handler.set_quality(bitrate);
  }
  ...
  set_default_bitrate() {
    if (this.handler.set_default_bitrate instanceof Function) this.handler.set_default_bitrate();
  }

  add_cm_listener(listener) {
    if (this.handler.add_cm_listener instanceof Function) this.handler.add_cm_listener(listener);
  }

  clear() {
    if (this.handler.clear instanceof Function) this.handler.clear();
  }
}
